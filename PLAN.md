...
4. **Build for arm64: done.** Both architectures are built and released. The wallet publishes an
   ...
   cross-compiling. The arm64 image is assembled under emulation, taking six to nine minutes
   against about five for the native build. What no build proves is that it runs: that waits on
   arm64 hardware.
